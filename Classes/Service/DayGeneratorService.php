...
class DayGeneratorService implements LoggerAwareInterface
{
    ...
    public function getDateTimeStorageForEvent(array $eventRecord): array
    {
        ...
        try {
            $this->addDateTimeObjectsToEventRecord($eventRecord);
     ...
     * @throws \Exception
     */
    protected function addDateTimeObjectsToEventRecord(array &$eventRecord): void
    {
        foreach ($eventRecord as $column => $value) {
            if (!isset($GLOBALS['TCA']['tx_events2_domain_model_event']['columns'][$column])) {
                continue;
            }

            if (!isset($GLOBALS['TCA']['tx_events2_domain_model_event']['columns'][$column]['config']['renderType'])) {
                continue;
            }

            if ($GLOBALS['TCA']['tx_events2_domain_model_event']['columns'][$column]['config']['renderType'] === 'inputDateTime') {
                $eventRecord[$column] = $this->dateTimeUtility->convert($value);
            }
        }
    }
