...
class DayGeneratorService
{
    ...
    protected function checkEventRecord(array $eventRecord): void
    {
        ...
        if ($eventRecord['event_begin']->format('H:i:s') !== '00:00:00') {
            throw new \Exception('Column "event_begin" was given with a time other than 00:00:00 which is not allowed.', 2689896730);
        }
        if (
            $eventRecord['event_end'] instanceof \DateTimeImmutable
            && $eventRecord['event_end']->format('H:i:s') !== '00:00:00'
        ) {
            throw new \Exception('Column "event_end" was given with a time other than 00:00:00 which is not allowed.', 7534858408);
        }
        if (
            $eventRecord['recurring_end'] instanceof \DateTimeImmutable
            && $eventRecord['recurring_end']->format('H:i:s') !== '00:00:00'
        ) {
            throw new \Exception('Column "recurring_end" was given with a time other than 00:00:00 which is not allowed.', 2544473663);
        }
    }
